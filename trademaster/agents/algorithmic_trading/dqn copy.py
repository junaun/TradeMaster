...
class AlgorithmicTradingDQN():
    def __init__(self, **kwargs):
        ...
        self.gamma = 0.9  # discount factor of future reward
        self.reward_scale = 1
        self.repeat_times = 1
        self.batch_size = 64

        self.clip_grad_norm = 3.0  # clip the gradient after normalization
        self.soft_update_tau = 0  # the tau of soft target update `net = (1-tau)*net + net1`
        self.state_value_tau = 5e-3  # the tau of normalize for value and state

        self.last_state = None  # last state of the trajectory for training. last_state.shape == (num_envs, state_dim)

        '''network'''
        self.act = self.act_target = dict(type='QNet', state_dim=159, action_dim=3, dims=(64, 32), explore_rate=0.25).to(self.device)
